...
public class AuthSync
{
	...
	public bool Start()
	{
		try
		{
			...
			CLogger.Print($"Auth Sync Address {ıPEndPoint.Address}:{ıPEndPoint.Port}", LoggerType.Info);
	...
	{
		if (bool_0 || AuthXender.Client == null || AuthXender.Client.ServerIsClosed)
		{
			return;
		}
	...
	private void method_2(byte[] byte_0)
	{
		try
		{
			...
			case 24:
				...
				break;
			default:
				CLogger.Print(Bitwise.ToHexData($"Auth - Opcode Not Found: [{num}]", syncClientPacket.ToArray()), LoggerType.Opcode);
				break;
			case 31:
