using System;
using System.Collections.Generic;
using Plugin.Core;
using Plugin.Core.Enums;
using Plugin.Core.Models;
using Plugin.Core.SQL;
using Server.Auth.Data.Models;
using Server.Auth.Network.ServerPacket;

namespace Server.Auth.Network.ClientPacket;

public class PROTOCOL_BASE_GET_SYSTEM_INFO_REQ : AuthClientPacket
{
	public override void Read()
	{
		ReadC();
	}

	public override void Run()
	{
		try
		{
			Account player = Client.Player;
			if (player == null)
			{
				return;
			}
			ServerConfig config = AuthXender.Client.Config;
			if (config != null)
			{
				Client.SendPacket(new PROTOCOL_BASE_NOTICE_ACK(config));
				if (config.OfficialBannerEnabled)
				{
					Client.SendPacket(new PROTOCOL_BASE_URL_LIST_ACK(config));
				}
				Client.SendPacket(new PROTOCOL_BASE_BOOSTEVENT_INFO_ACK());
				Client.SendPacket(new PROTOCOL_BASE_CHANNELTYPE_CONDITION_ACK());
				Client.SendPacket(new PROTOCOL_BASE_GET_SYSTEM_INFO_ACK(config));
				Client.SendPacket(new PROTOCOL_BASE_BATTLE_MODE_WEAPON_INFO_ACK());
			}
			if (!player.MyConfigsLoaded)
			{
				Client.SendPacket(new PROTOCOL_BASE_GET_OPTION_ACK(0, player.Config));
			}
			List<MessageModel> giftMessages = DaoManagerSQL.GetGiftMessages(player.PlayerId);
			if (giftMessages.Count > 0)
			{
				DaoManagerSQL.RecycleMessages(player.PlayerId, giftMessages);
				if (giftMessages.Count > 0)
				{
					Client.SendPacket(new PROTOCOL_BASE_USER_GIFTLIST_ACK(0, giftMessages));
				}
			}
		}
		catch (Exception ex)
		{
			CLogger.Print(ex.Message, LoggerType.Error, ex);
		}
	}
}
