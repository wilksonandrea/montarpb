...
public class PROTOCOL_SHOP_GET_SAILLIST_REQ : GameClientPacket
{
	...
	public override void Run()
	{
		try
		{
			...
			if (Bitwise.ReadFile(Environment.CurrentDirectory + "/Data/Raws/Shop.dat") == string_0)
			{
				Client.SendPacket(new PROTOCOL_SHOP_GET_SAILLIST_ACK(bool_1: false));
			}
			else
			{
				Client.SendPacket(new PROTOCOL_SHOP_GET_SAILLIST_ACK(bool_1: true));
			}
		}
		catch (Exception ex)
		{
