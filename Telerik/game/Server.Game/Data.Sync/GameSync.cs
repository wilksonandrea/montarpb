	...
	public class GameSync
	{
		...
		public void Close()
		{
			...
			catch (Exception exception1)
			{
				...
				CLogger.Print(string.Concat("Error closing GameSync: ", exception.Message), LoggerType.Error, exception);
			}
		}
		...
		private void method_2(byte[] byte_0)
		{
			try
			{
				...
				switch (ınt16)
				{
					...
					case 30:
					{
						CLogger.Print(Bitwise.ToHexData(string.Format("Game - Opcode Not Found: [{0}]", ınt16), syncClientPacket.ToArray()), LoggerType.Opcode, null);
						break;
					}
					case 10:
					{
					...
					case 31:
					{
						Server.Game.Data.Sync.Client.EventInfo.LoadEventInfo(syncClientPacket);
						break;
					}
					case 32:
					{
				...
				if (Packet != null)
				{
					...
					if (server != null)
					{
						...
						using (SyncServerPacket syncServerPacket = new SyncServerPacket())
						{
							...
							syncServerPacket.WriteH((ushort)((int)bytes.Length));
				...
				if (Data.Length != 0)
				{
					...
					if (server != null)
					{
						...
						using (SyncServerPacket syncServerPacket = new SyncServerPacket())
						{
							...
							syncServerPacket.WriteH((ushort)((int)Data.Length));
		...
		public bool Start()
		{
			...
			try
			{
				...
				CLogger.Print(string.Format("Game Sync Address {0}:{1}", localEndPoint.Address, localEndPoint.Port), LoggerType.Info, null);
