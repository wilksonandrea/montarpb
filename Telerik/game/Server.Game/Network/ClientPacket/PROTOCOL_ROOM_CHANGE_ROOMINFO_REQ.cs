		...
		public override void Run()
		{
			...
			try
			{
				...
				if (player != null)
				{
					...
					{
						bool flag1 = !room.Name.Equals(this.string_0);
						bool flag2 = (room.Rule != this.mapRules_0 || room.Stage != this.stageOptions_0 ? true : room.RoomType != this.roomCondition_0);
						...
						roomModel.WatchRuleFlag = (byte)byte3;
						...
						if (this.roomState_0 < RoomState.READY)
						{
							flag = true;
						}
						else
						{
							flag = (this.string_1.Equals("") ? true : !this.string_1.Equals(player.Nickname));
						}
						if (flag | flag1 | flag2 || this.roomWeaponsFlag_0 != room.WeaponsFlag || this.int_1 != room.CountMaxSlots)
						{
						...
						room.UpdateRoomInfo();
						using (PROTOCOL_ROOM_CHANGE_ROOM_OPTIONINFO_ACK pROTOCOLROOMCHANGEROOMOPTIONINFOACK = new PROTOCOL_ROOM_CHANGE_ROOM_OPTIONINFO_ACK(room))
						{
							room.SendPacketToPlayers(pROTOCOLROOMCHANGEROOMOPTIONINFOACK);
						}
					}
				}
			}
			catch (Exception exception1)
			{
				...
				CLogger.Print(string.Concat("PROTOCOL_BATTLE_CHANGE_ROOMINFO_REQ: ", exception.Message), LoggerType.Error, exception);
			}
		}
	}
}