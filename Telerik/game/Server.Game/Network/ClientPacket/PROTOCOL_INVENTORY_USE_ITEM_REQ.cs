		...
		private void method_0(int int_0, uint uint_2, Account account_0)
		{
			...
			else if (int_0 != 1600052)
			{
				...
				if (int_0 == 1600006)
				{
					...
					if (account_0.Room != null)
					{
						using (PROTOCOL_ROOM_GET_COLOR_NICK_ACK pROTOCOLROOMGETCOLORNICKACK = new PROTOCOL_ROOM_GET_COLOR_NICK_ACK(account_0.SlotId, account_0.NickColor))
						{
							account_0.Room.SendPacketToPlayers(pROTOCOLROOMGETCOLORNICKACK);
						}
						account_0.Room.UpdateSlotsInfo();
						return;
					}
			...
			try
			{
				...
				if (player != null)
				{
					...
					else
					{
						...
						dateTime = dateTime.AddDays((double)ComDiv.GetIdStatics(ıtem.Id, 2));
						uint uInt32 = Convert.ToUInt32(dateTime.ToString("yyMMddHHmm"));
						if (ınt32 != 1600047 && ınt32 != 1600051)
						{
							if (ınt32 == 1600010)
							{
								goto Label3;
					...
					Label0:
						this.method_0(ınt32, uInt32, player);
					}
					this.Client.SendPacket(new PROTOCOL_AUTH_SHOP_ITEM_AUTH_ACK(this.uint_1, ıtem, player));
				}
			}
			catch (Exception exception1)
			{
				Exception exception = exception1;
				CLogger.Print(string.Concat("PROTOCOL_INVENTORY_USE_ITEM_REQ: ", exception.Message), LoggerType.Error, exception);
			}
		}
	}
}