...
namespace Server.Auth.Network.ServerPacket
{
	...
	{
		// Token: 0x06000079 RID: 121 RVA: 0x00005684 File Offset: 0x00003884
		...
		// Token: 0x0600007A RID: 122 RVA: 0x00005734 File Offset: 0x00003934
		public override void Write()
		{
			...
			if (this.uint_0 == 0U)
			{
				...
				base.WriteC((byte)((this.list_1.Count == 0) ? 0 : this.account_0.Character.GetCharacter(this.playerEquipment_0.CharaRedId).Slot));
		...
		// Token: 0x0600007B RID: 123 RVA: 0x000063BC File Offset: 0x000045BC
		private byte[] method_0(Account account_1, EventVisitModel eventVisitModel_1, uint uint_2)
		{
			...
			using (SyncServerPacket syncServerPacket = new SyncServerPacket())
			{
				...
				{
					uint num = uint.Parse(string.Format("{0:yyMMdd}", DateTimeUtil.Convert(string.Format("{0}", uint_2))));
					...
					syncServerPacket.WriteC((num2 < num) ? 1 : 2);
		...
		private byte[] method_1(Account account_1, EventVisitModel eventVisitModel_1)
		{
			...
			using (SyncServerPacket syncServerPacket = new SyncServerPacket())
			{
				...
				if (eventVisitModel_1 != null && eventVisitModel_1.EventIsEnabled())
				{
					...
					syncServerPacket.WriteD((event2 != null) ? event2.BeginDate : 0U);
					syncServerPacket.WriteD((event2 != null) ? event2.EndedDate : 0U);
					syncServerPacket.WriteD(0);
					for (int i = 0; i < 31; i++)
					{
						VisitBoxModel visitBoxModel = eventVisitModel_1.Boxes[i];
						syncServerPacket.WriteC((visitBoxModel.IsBothReward > false) ? 1 : 0);
		...
		// Token: 0x0600007D RID: 125 RVA: 0x0000661C File Offset: 0x0000481C
		private byte[] method_2(List<QuickstartModel> list_2)
		{
			...
			using (SyncServerPacket syncServerPacket = new SyncServerPacket())
			{
				...
				foreach (QuickstartModel quickstartModel in list_2)
				{
		...
		// Token: 0x0600007E RID: 126 RVA: 0x000066CC File Offset: 0x000048CC
		...
		// Token: 0x0600007F RID: 127 RVA: 0x00006734 File Offset: 0x00004934
