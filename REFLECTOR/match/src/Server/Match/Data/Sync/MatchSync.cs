    ...
    public class MatchSync
    {
        ...
        public void Close()
        {
            if (!this.bool_0)
            {
                ...
                {
                    CLogger.Print("Error closing MatchSync: " + exception.Message, LoggerType.Error, exception);
                }
            }
        }
        ...
        private void method_2(byte[] byte_0)
        {
            try
            {
                ...
                switch (num)
                {
                    ...
                    case 2:
                        RemovePlayerSync.Load(c);
                        break;

                    case 3:
                        MatchRoundSync.Load(c);
                        break;

                    default:
                        CLogger.Print(Bitwise.ToHexData($"Match - Opcode Not Found: [{num}]", c.ToArray()), LoggerType.Opcode, null);
                        break;
                }
            }
            catch (Exception exception)
        ...
        public void SendPacket(byte[] Data, IPEndPoint Address)
        {
            if (!this.bool_0)
            {
                ...
                {
                    CLogger.Print($"MatchSync socket disposed during SendPacket to {Address}.", LoggerType.Warning, null);
                }
                catch (Exception exception)
                {
        ...
        public bool Start()
        {
            try
            {
                ...
                CLogger.Print($"Match Sync Address {localEndPoint.Address}:{localEndPoint.Port}", LoggerType.Info, null);
