    ...
    public class GameSync
    {
        ...
        public void Close()
        {
            if (!this.bool_0)
            {
                ...
                {
                    CLogger.Print("Error closing GameSync: " + exception.Message, LoggerType.Error, exception);
                }
            }
        }

        public SChannelModel GetServer(AccountStatus status) => 
            this.GetServer(status.ServerId);

        public SChannelModel GetServer(int serverId) => 
            ((serverId == 0xff) || (serverId == GameXender.Client.ServerId)) ? null : SChannelXML.GetServer(serverId);
        ...
        private void method_2(byte[] byte_0)
        {
            try
            {
                ...
                CLogger.Print(Bitwise.ToHexData($"Game - Opcode Not Found: [{num}]", c.ToArray()), LoggerType.Opcode, null);
            }
            catch (Exception exception)
            {
                ...
                if (Packet != null)
                {
                    ...
                    if (server != null)
                    {
                        ...
                        using (SyncServerPacket packet = new SyncServerPacket())
                        {
                            ...
                            packet.WriteC(0);
                            packet.WriteC((byte) (name.Length + 1));
                            packet.WriteS(name, name.Length + 1);
                            packet.WriteH((ushort) bytes.Length);
                            packet.WriteB(bytes);
                            this.SendPacket(packet.ToArray(), connection);
                        }
                    }
                }
            }
                ...
                if (Data.Length != 0)
                {
                    ...
                    if (server != null)
                    {
                        ...
                        using (SyncServerPacket packet = new SyncServerPacket())
                        {
                            ...
                            packet.WriteQ(PlayerId);
                            packet.WriteC(0);
                            packet.WriteC((byte) (PacketName.Length + 1));
                            packet.WriteS(PacketName, PacketName.Length + 1);
                            packet.WriteH((ushort) Data.Length);
                            packet.WriteB(Data);
                            this.SendPacket(packet.ToArray(), connection);
                        }
                    }
        ...
        public void SendPacket(byte[] Data, IPEndPoint Address)
        {
            if (!this.bool_0)
            {
                ...
                {
                    CLogger.Print($"GameSync socket disposed during SendPacket to {Address}.", LoggerType.Warning, null);
                }
                catch (Exception exception)
                {
        ...
        public bool Start()
        {
            try
            {
                ...
                CLogger.Print($"Game Sync Address {localEndPoint.Address}:{localEndPoint.Port}", LoggerType.Info, null);
