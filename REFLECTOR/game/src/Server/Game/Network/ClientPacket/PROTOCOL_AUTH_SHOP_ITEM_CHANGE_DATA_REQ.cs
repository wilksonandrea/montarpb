        ...
        public override void Run()
        {
            try
            {
                ...
                if (player != null)
                {
                    ...
                    if ((model == null) || (model.Id <= 0x186a00))
                    {
                        this.uint_1 = 0x80000000;
                    }
                    else
                    {
                        ...
                        if ((num == 0x18913f) || ((num == 0x189143) || (num == 0x186a0a)))
                        {
                            this.string_0 = Bitwise.HexArrayToString(this.byte_0, this.byte_0.Length);
                        }
                        else if ((num == 0x189144) || (num == 0x186a05))
                        {
                    ...
                    base.Client.SendPacket(new PROTOCOL_AUTH_SHOP_ITEM_CHANGE_DATA_ACK(this.uint_1));
                }
            }
            catch (Exception exception)
            {
                CLogger.Print("PROTOCOL_AUTH_SHOP_ITEM_CHANGE_DATA_REQ: " + exception.Message, LoggerType.Error, exception);
            }
        }
    }
}
